...
if selected == "Order Delivery Status":
    ...
    with st.expander("Interpretasi", expanded=False):
        # Konten expander
        st.text("""- Hampir ebagian besar produk yaitu sejumlah 106893 produk yang terjual berhasil sampai ke pelanggan dengan tepat waktu.
...
if selected == "Payment Type":
    ...
    with st.expander("Interpretasi", expanded=False):
            ...
            # Konten expander
            st.text("""- Jenis pembayaran yang paling banyak digunakan pelanggan adalah credit card dan juga boleto dengan jumlah transaksi sebanyak 85277 dan 22510 transaksi pada masing - masing jenis pembayaran tersbut.
    ...
    with st.expander("Saran", expanded=False):
            ...
            # Konten expander
            st.text("""Promosi dan Insentif : 
            - Untuk mendorong penggunaan jenis pembayaran yang paling sedikit digunakan, Dapat dibuat promosi atau memberikan insentif khusus. Misalnya, memberikan diskon tambahan atau penawaran eksklusif bagi pelanggan yang menggunakan debit card.""")
...
if selected == "Purchase Time":
    ...
    with st.expander("Interpretasi", expanded=False):
            ...
            # Konten expander
            st.text("""- Waktu transaksi pelanggan paling banyak dilakukan pada pagi hari yaitu sebesar 44871 transaksi.
    ...
    with st.expander("Saran", expanded=False):
            ...
            # Konten expander
            st.text("""Penawaran di Waktu Yang Ramai Pelanggan :
            - Perhatikan waktu yang paling populer untuk transaksi, yaitu pagi hari. Kirimkan pemberitahuan atau promosi yang relevan kepada pelanggan pada waktu-waktu tersebut. Misalnya, mengirimkan pesan atau penawaran khusus yang dikirim tepat sebelum atau selama jam transaksi yang paling di pagi hari. Hal ini akan memaksimalkan kesempatan Anda untuk menarik perhatian pelanggan dan mendorong mereka untuk melakukan pembelian.
            
            Promosi sesuai dengan waktu aktif pelanggan :
            - Mengirimkan promosi yang lebih disesuaikan dan relevan kepada pelanggan pada waktu preferensi pelanggan. Misalnya, jika Ada  pelanggan yang lebih aktif pada siang hari, Maka dapat dikirimkan penawaran eksklusif atau informasi produk terbaru saat siang hari. Sehingga, terjadi kemungkinan interaksi dan konversi transaksi pada pelanggan lebih tinggi.""")
